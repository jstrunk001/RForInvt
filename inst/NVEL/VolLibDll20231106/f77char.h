...
/*
   ...
   also done each time a string array is subscipted using
...
:rep(cstring), len(strlen(cstring))
{
   // find position from where to sart padding
...
{
   if(rep[len] == '\0')
      return;                    // chatches string constants
   ...
   for(int i=len-1;i>=0;i--)
   {
      ...
      {
         rep[i+1] = '\0';        // zero terminated and jump out
         break;                  
      }
   }
}
