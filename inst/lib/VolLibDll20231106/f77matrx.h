...
/*
   ...
   and subsequently returned back to C++ as prperly arranged C++ arrays.

   Problem:  FORTRAN oranizes data in a "column first" order,
            C++ organizes data in a "row-first" order.
...
class FMATRIX
{
   public:
      ...
      ~FMATRIX();
   public:
      const size_t ndim;   // number of array dimnesions
      size_t dim[2];       // size of each dimension
...
{
   assert(ndim==2);     // only 2d arrays supported

   // indexing according to F77 conventions
   size_t index_f77 = index2*dim[0] + index1;

   // return a refference to the array element
...
FMATRIX<T>::~FMATRIX()
{
   ...
   //delete the FORTRAN copy of the array
   delete[] f77rep;
}
