...
test_that("NVEL_buck1 returns correct column prefixes and structure", {
  ...
  res <- NVEL_buck1(dfTL = df_test, vol2biomass = TRUE)
...
test_that("NVEL_buck1 handles non-merchantable trees (Zero Logs)", {
...
test_that("NVEL_buck1 correctly expands a large tree into multiple logs", {
...
test_that("NVEL_buck1 biomass columns are added when vol2biomass is TRUE", {
  df_test <- data.frame(
    region = 6, forest = "01", district = "01",
    spcd = 202, dbh = 10, ht = 60
  )

  res_with <- NVEL_buck1(dfTL = df_test, vol2biomass = TRUE)
  res_without <- NVEL_buck1(dfTL = df_test, vol2biomass = FALSE)

  expect_true("TR_TBIOGRN_LBS" %in% names(res_with))
  expect_false("TR_TBIOGRN_LBS" %in% names(res_without))
})

test_that("NVEL_buck1 handles custom column names correctly", {
...
test_that("NVEL_buck1 voleq parameter overrides default equations", {
  df_test <- data.frame(
  ...
  forced_eq <- "F0162521"
  res <- NVEL_buck1(dfTL = df_test, voleq = forced_eq)
