...
#'          library(RSForInvt)
#'          NVEL_nvbeq(region = 2, forest = "01",district = "01", spcd=951)
#'          NVEL_nvbeq(region = 2, forest = "01",district = "01", spcd=951)
#'          NVEL_nvbeq(region = 2, forest = "01",district = "01", spcd=rep(c(951,201),2))
#'          NVEL_nvbeq(dfTL=data.frame(region = 6, forest = "01",district = "01", spcd=rep(c(951,201),2)))
#'
...
if(F){
  ...
  if(!"dfSpp" %in% ls()){
    ...
    library(RSQLite)
    system.file('lib/VolLibDll20231106/vollib-64bits/vollib.dll', package="RForInvt")
