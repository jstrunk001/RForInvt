...
#browser()
	...
	if(nclus >1){
		...
		#process plots
		res_i = parLapply(clus_in , spl_IDs , .compile_1plot , trs = dat_in,  trNms = dfTreeNms , plotNms = dfPlotNms, fnCompute = fnCompute , ... )

		#debugging code
		if(F){
		  res_i = lapply(spl_IDs[1:3] , .compile_1plot , trs = dat_in,  trNms = dfTreeNms , plotNms = dfPlotNms, fnCompute = fnCompute , ... )
		  res_i = parLapply(clus_in, spl_IDs[1:3] , .compile_1plot , trs = dat_in,  trNms = dfTreeNms , plotNms = dfPlotNms , fnCompute = fnCompute , ... )
		  res_i = parLapply(clus_in, spl_IDs[1:3] , .compile_1plotB , fnCompute = fnCompute , ... )
		}
...
.compile_1plot=function(
  ...
  trs_i = merge(y=trs ,x=id, by = trNms[["plotIDs"]], all.x = T, all.y=F)
...
sppYplot = function(
  ...
  #                          ,dfPlotNms = list(plotIDs = c("SP_CN","PLOT","SUBP", "INVYR"), pltWt = NA),
  #                           ,dir_out = dir_out,
  #                           ,nm_out = "subplot_compile_parallel",
  #                           ,fnCompute = list(plotWtSum)
  #                           ,return = TRUE
  #                           ,doDebug = TRUE
  #                           ,nclus = 3
  #                           ,vSumNm = nms_compile1
