...
#'@param dfplot_nms map expected plot column names onto dfPlot. These are the expected names: c( plotIDs = c("?","?"), pltWt = c(NA,"?")[1] ). Feel free to provide others used by custom functions.
...
#'
#'
#'@import plyr parallel sqldf reshape2 RSQLite
#'
...
compilePlots=function(
  ...
  ,dfplot_nms = list(plotIDs = c( "STATE" , "COUNTY" , "PROJECT" , "PLOT" , "YEAR" ), pltWt = NA )
  ...
  if(!is.na(dir_out)) path_sqlite = file.path(dir_out,paste0(nm_out,".sqlite"))
  ...
  dir_out = dir_out[1]
	...
	if(!no_dfPlot){
  	pl_ids_ok = dfplot_nms[["plotIDs"]] %in% names(dfPlot)
  	if(mean(pl_ids_ok) < 1 ) stop(paste("dfPlot does not have some plotIDs:",dfplot_nms[["plotIDs"]][!pl_ids_ok]))
	}
...
#browser()
	...
	if(nclus == 1){
		...
		res_i = lapply(spl_IDs , .compile_1plot , trs = dat_in,  tr_nms = dfTreeNms , plot_nms = dfplot_nms, fns_compute = fns_compute ,dir_out=dir_out , ... )

		if(F) res_i = lapply(spl_IDs[1:3] , .compile_1plot , trs = dat_in,  tr_nms = dfTreeNms , plot_nms = dfplot_nms, fns_compute = fns_compute ,dir_out=dir_out, ... )
	...
	if(nclus >1){
 		...
 		clus_in=parallel::makeCluster(nclus)
  		...
  		#process plots
  		res_i = parLapply(clus_in , spl_IDs , .compile_1plot , trs = dat_in,  tr_nms = dfTreeNms , plot_nms = dfplot_nms, fns_compute = fns_compute ,dir_out=dir_out, ... )
...
.compile_1plot=function(
  ...
  trs_i = merge(y=trs ,x=id, by = tr_nms[["plotIDs"]], all.x = T, all.y=F)

  #get grouping variables as seed columns with id fields

    trs_ID = trs_i[1, tr_nms[["plotIDs"]]]
    ...
    ll = flock::lock(lock.name)
      ...
      err_i=try(dbWriteTable(path_sqlite,tb_csv,res_in,append=T))
...
plotWtSum = function(
	...
	bad_ids = is.na(trs[,tr_nms[["plotIDs"]]])
	tr_in = trs[!bad_ids,]
	...
	if(nrow(tr_in) > 0){
		sum_in = lapply( sumNm_in , function(colNm,wtNm,x,...) data.frame( sum(x[,colNm]*x[,wtNm],na.rm=T)), wtNm = tr_nms[["trWt"]] , x = tr_in )
...
plotWtMn = function(
	...
	bad_ids = apply(is.na(trs[,tr_nms[["plotIDs"]],drop=F]),1,function(x) TRUE %in% x )
	tr_in = trs[!bad_ids,]

	#correct for NA weights
	tr_in[is.na(tr_in[,tr_nms[["dbh"]]]),] = 0
	tr_in[is.na(tr_in[,tr_nms[["trWt"]]]),] = 0

	if(nrow(tr_in) > 0){

		wtmn = data.frame(
			ntree = nrow(tr_in)
			,qmd = sqrt(sum(tr_in[,tr_nms[["trWt"]]]*tr_in[,tr_nms[["dbh"]]]^2,na.rm=T) / sum(tr_in[,tr_nms[["trWt"]]],na.rm=T))
			,lorht = sum(tr_in[,tr_nms[["ht"]]] * tr_in[,tr_nms[["trWt"]]] * tr_in[,tr_nms[["dbh"]]]^2,na.rm=T)/ sum(tr_in[,tr_nms[["trWt"]]] * tr_in[,tr_nms[["dbh"]]]^2,na.rm=T)
		)
...
sppYplot = function(
  ...
  bad_ids = apply(is.na(trs[,tr_nms[["plotIDs"]],drop=F]),1,function(x) TRUE %in% x )
  if(sum(bad_ids) > 0){
    warning("some records have bad plotIDs fields:", unique(trs[bad_ids,tr_nms[["plotIDs"]]]) )
  }
  tr_in = trs[!bad_ids,]

  #correct for NA weights
  tr_in[is.na(tr_in[,tr_nms[["dbh"]]]),tr_nms[["dbh"]]] = 0
  tr_in[is.na(tr_in[,tr_nms[["trWt"]]]),tr_nms[["trWt"]]] = 0

  #get data holder for results
  res_in = tr_in[1,tr_nms[["plotIDs"]],drop=F]

  #iterate across response fields
  for(i in 1:length(tr_nms[["domSppY"]])){

    #compute weighted values
    if(!is.na(tr_nms[["trWt"]])) tr_in[,tr_nms[["domSppY"]][i]] = tr_in[,tr_nms[["domSppY"]][i]] * tr_in[,tr_nms[["trWt"]]]

    #cast and aggregate
    mi = reshape2::melt( tr_in[,c(tr_nms[["plotIDs"]],tr_nms[["spcd"]],tr_nms[["domSppY"]][i]) ] , id.vars = c(tr_nms[["plotIDs"]],tr_nms[["spcd"]]) )
    fi = as.formula(paste("variable  + ", paste(tr_nms[["plotIDs"]],collapse = "+")," ~ ",tr_nms[["spcd"]], sep=""))
    ...
    spp_nms = names(dfi)[-1]
    nmsMx = paste("dom", tr_nms[["spcd"]], tr_nms[["domSppY"]][i],1:n_dom, sep="_")
    nmsMxProp = paste("dom_prop", tr_nms[["spcd"]], tr_nms[["domSppY"]][i],1:n_dom, sep="_")
    dfi[,nmsMx] = spp_nms[dom_order][1:n_dom]

    #get proportion by species
    nmsMx_p = paste(spp_nms, tr_nms[["domSppY"]][i],"p", sep="_")
    y_ord_ndom = dfi1[,-1][dom_order][1:n_dom]
    dfi[,nmsMx_p] = y_ord_ndom / sum(y_ord_ndom)

    #merge data
    res_in = merge(res_in, dfi[,c(tr_nms[["plotIDs"]],nmsMx,nmsMx_p)] , by=tr_nms[["plotIDs"]])
  ...
  # if(!"cl2" %in% ls()) cl2 = makeCluster(2)
  #
  # test_plots = compilePlots(
  #                         cl2
  #                         ,dfTree = trs_test
  #                         ,dfPlot = sp_test
  #                         ,dfTreeNms = list(plotIDs = c("SP_CN","PLOT","SUBP", "INVYR")
  #                           ,trIDs = c("CN") , dbh = "DIA", ht = "HT", spcd = "SPCD", trWt = "TPA_UNADJ1")
  #                          ,dfplot_nms = list(plotIDs = c("SP_CN","PLOT","SUBP", "INVYR"), pltWt = NA),
  #                           ,dir_out = dir_out,
  #                           ,nm_out = "subplot_compile_parallel",
  #                           ,fns_compute = list(plotWtSum)
  #                           ,return = TRUE
  #                           ,doDebug = TRUE
  #                           ,nclus = 3
  #                           ,vSumNm = nms_compile1
...
#'     wtmn = data.frame(
#'       ntree = nrow(tr_in)
#'       ,ba_ftac = sum(.005454*tr_in[,tr_nms[["dbh"]]]^2*tr_in[,tr_nms[["trWt"]]],na.rm=T)
#'       ,ba_ftac_ge3 = sum((.005454*tr_in[,tr_nms[["dbh"]]]^2*tr_in[,tr_nms[["trWt"]]])[tr_in[,tr_nms[["dbh"]]] > 3],na.rm=T)
#'       ,ba_m3ha_ge3 = sum((.005454*tr_in[,tr_nms[["dbh"]]]^2*tr_in[,tr_nms[["trWt"]]])[tr_in[,tr_nms[["dbh"]]] > 3],na.rm=T)*(0.3048^2)/0.404686
#'       ,qmd = sqrt(sum(tr_in[,tr_nms[["trWt"]]]*tr_in[,tr_nms[["dbh"]]]^2,na.rm=T) / sum(tr_in[,tr_nms[["trWt"]]],na.rm=T))
#'       ,lorht = sum(tr_in[,tr_nms[["ht"]]] * tr_in[,tr_nms[["dbh"]]]^2,na.rm=T)/ sum(tr_in[,tr_nms[["dbh"]]]^2,na.rm=T)
#'     )
#'
#'   }else{
#'
...
#       #, dfPlot = data.frame()
#     )
#     ,tr_nms = c( trIDs = c("UNQ_TR") , plotIDs = "Plot" , dbh = "DIA" , ht = "HT" , spcd = "SPCD" , trWt = "TPA" )
#     ,plot_nms = c(plotIDs = c( "STATE" , "COUNTY" , "PROJECT" , "PLOT" , "YEAR" ) )
#
#     ,plot_filter = c(NA, "select * from dfPlot where YEAR = 2018 and STATE = 'WA' and CONDITION = 1")
#     ,tree_filter = c(NA, "select * from dfTree where dbh > 2 ")
#
...
#
#     ,fns_compute = list(
#       plotWtMn(dfTree_in , tr_nms , dbh_units = c("in","cm") , ht_units = c("ft","m") )
#       ,plotWtSum( dfTree_in , tr_nms , vSumNm = c('DRYBIOM',"VOLBFNET" , "VOLCFGRS", "VOLCFNET", "CARBON_AG", "DRYBIO_AG","DRYBIOT") )
