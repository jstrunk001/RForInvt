...
#'
#'  \dontrun{
#'    archive_table(mtcars, "output/mydata", table_nm = "CarsData")
#'    archive_table(mtcars, "output/mydata", do_rds = FALSE, do_sqlite = FALSE)
#'  }
#'
...
archive_table <- function(data,
  ...
  stopifnot(is.logical(do_csv), length(do_csv) == 1)
  stopifnot(is.logical(do_rds), length(do_rds) == 1)
  stopifnot(is.logical(do_sqlite), length(do_sqlite) == 1)
  stopifnot(is.logical(do_xlsx), length(do_xlsx) == 1)
  stopifnot(is.logical(increment), length(increment) == 1)
  ...
  if (do_csv && "csv" %in% names(extensions) && !is.na(extensions[["csv"]])) {
    path_out_csv <- file_version(paste0(path_out, extensions[["csv"]]), increment = increment)
  ...
  if (do_rds && "rds" %in% names(extensions) && !is.na(extensions[["rds"]])) {
    path_out_rds <- file_version(paste0(path_out, extensions[["rds"]]), increment = increment)
  ...
  if (do_sqlite && "sqlite" %in% names(extensions) && !is.na(extensions[["sqlite"]])) {
    path_out_sql <- file_version(paste0(path_out, extensions[["sqlite"]]), increment = increment)
  ...
  if (do_xlsx && "xlsx" %in% names(extensions) && !is.na(extensions[["xlsx"]])) {
    path_out_xlsx <- file_version(paste0(path_out, extensions[["xlsx"]]), increment = increment)
