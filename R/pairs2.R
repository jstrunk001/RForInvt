...
pairs2=function(
          ...
          if(combination){
            ...
            mapply(.plot_ij, nm_combn[,1] ,nm_combn[,2] , MoreArgs = list(x=x,y=y,mai_edge, ncols=ncols, nrows=nrows,oneLine=oneLine,str_max=str_max))
...
.plot_ij=function( i, j , y , x , mai_edge , ncols, nrows,oneLine,str_max){
  ...
  if( i==1 ){ mtext(substr(names(y)[j],1,str_max),side = 2, line = 1)}
