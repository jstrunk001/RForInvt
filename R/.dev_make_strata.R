...
make_strata = function(
  ...
  #get variable classes
	x1_in = x1
	x2_in = x2
	type_x1 = class(data[[x1_in]])
	type_x2 = class(data[[x2_in]])

	if( !type_x1 == "numeric"){

	  str_levels_x1 = levels(as.factor(data[[x1_in]]))
	...
	if( type_x1 == "numeric"){
	  ...
	  dfStr0 = read.csv(text=paste("stratum_x1,x1.from,x1.to,n_x1"))
		...
		if(split_x1[1] == "qt" ){
			...
			str_levels_x1 = c(
				r_x1[1]-dr_x1
				,quantile(data[[x1_in]] ,seq(1/n1,1-1/n1,1/n1))
				,r_x1[1]+dr_x1
			)
		}
		if(split_x1[1] == "eq" ){
		...
		x1_in_ctg = paste(x1_in,"_fct",sep="")
		data[,x1_in_ctg] = cut(data[[x1_in]] , str_levels_x1 , labels = 1:(length(str_levels_x1)-1) )
...
browser()
  ...
  if( nest_x2 & type_x2 == "numeric"){
    ...
    spl1 = split(data,data[[x1_in_ctg]])
    ...
    if( type_x2 == "numeric"){
      ...
      for(i in 1:length(spl1)){
        ...
        if(split_x2[1] == "qt" ){
          ...
          str_levels_x2 = c(
            ri[1]-dri
            ,quantile(dati[[x2_in]] ,seq(1/n2i,1-1/n2i,1/n2i))
            ,ri[2]+dri
          )
        }
        if(split_x2[1] == "eq" ){
        ...
        dfStri[, "stratum_x1" ] =  dati[[x1_in]][1]
        ...
        df_ct = as.data.frame(table(as.factor(cut(dati[[x2_in]], round(str_levels_x2,precision) , labels = F ))))
	...
	if(!nest_x2 & type_x2 == "numeric"){
	  ...
	  if(split_x2[1] == "qt" ){
	    ...
	    str_levels_x2 = c(
	      r_x2[1]-dr_x2
	      ,quantile(data[[x2_in]] ,seq(1/n1,1-1/n1,1/n1))
	      ,r_x2[1]+dr_x2
	    )
	  }
	  if(split_x2[1] == "eq" ){
	  ...
	  #prepare new factor version of attribute
	  x2_in = paste(x2_in,"_fct",sep="")
	  data[[x2_in]] = cut(data[[x2_in]] , str_levels_x2 , labels = 1:(length(str_levels_x2)-1) )
	...
	if( type_x2 == "factor"){
	  ...
	  str_levels_x1b = levels(as.factor(dfStr0[,"stratum_x1"]))
	  str_levels_x2 = levels(as.factor(data[[x2_in]]))
  ...
  if(collapse){
    browser()
    pdstrat_in = assign_strata(dfStr2,cbind(data[[x1_in]],data[[x2_in]]))$stratum
