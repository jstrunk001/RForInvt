...
#'@examples
#'
#'  sample_systematic(2000, 25)
#'  sample_systematic(205, n=c(50,50))
#'
#'
#'@export
#
...
sample_systematic = function(
  ...
  if(length(n)>1){
    ...
    for(j in 1:n_to){
      #take sample
      Nj = sum(idx==0)
      idx_j = sample_systematic( N=Nj , n=n[j] , exact_n=exact_n )
  ...
  if(length(n) == 1){
   ...
   #subsample idx to size n, or add missing idx with random supplement
    ...
    if(as_idx && length(idx) > 0 && max(idx) > N){
      stop("internal error: sampled index exceeds population size N")
    }
