...
model_predict = function(
  ...
  for (i in seq_len(nrow(params_in))){
    ...
    } else if (class_i == "nvel"){
      # DLL-backed NVEL volume equation: no fit object / archive root needed;
      # the "model" is the stored voleq / species selector.
      pdi = try(.mp_predict_nvel(dat = dat_in, model_row = model_i), silent = TRUE)
